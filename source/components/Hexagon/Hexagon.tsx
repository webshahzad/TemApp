...
export function Hexagon(props: HexagonProps): React.ReactElement {
...
const interests = App.interests;
    ...
    if (cells) {
      for (const index in cells) {
        if (cells.hasOwnProperty(index)) {
          ...
          customizations[indexFromStart] = cells[index]
  ...
  return (
    <View onLayout={onLayout}>
      <Svg width='120%' height={svgHeight}>
        <Defs>
          {hexDefinitions}
    ...
    result = (
      <ForeignObject key={`foreign-object-render-${cellContentAndForeignObjectCounter}`}>
        <View style={[styles.cellContent, { height: hexHeight, width: hexWidth }]}>
          {content.image !== undefined ? (<Image source={content.image as number} onLoadEnd={onImageLoadEnd} tintColor={content.tintColor} mainAxisSize={23} style={styles.image} />) : undefined}
